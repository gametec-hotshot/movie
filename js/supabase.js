...
const SupabaseSync = {
    ...
    async pullAllFromCloud() {
        ...
        try {
            const { data: cloudCW, error } = await supabaseClient.from('continue_watching').select('*').eq('user_id', user.id).order('updated_at', { ascending: false });
            if (error) {
                console.error("Supabase RLS/Select Error for continue_watching:", error);
                // Wait for a moment so the user can see this in console if they are debugging
            }
            if (cloudCW && cloudCW.length > 0) {
                let cw = JSON.parse(localStorage.getItem('continue_watching')) || [];

                // Process in reverse order (oldest first) so that the newest items are unshifted LAST,
                // leaving them safely at the front of the local 'cw' array.
                for (let i = cloudCW.length - 1; i >= 0; i--) {
                    const cloudItem = cloudCW[i];
                    ...
                    let progData = {
                        ...
                        poster_path: cloudItem.poster_path || null,
                        updatedAt: cloudItem.updated_at ? new Date(cloudItem.updated_at).getTime() : Date.now()
                    };
                    ...
                    } else if (!existing.updatedAt || existing.updatedAt < progData.updatedAt) {
    ...
    async loadTraktTokens() {
        ...
        try {
            ...
            if (data && data.access_token) {
                ...
                let expiresIn = 7776000; // default 90 days
                ...
                if (data.expires_at) {
                    ...
                    if (diffSeconds > 0) {
                        ...
                        expiresIn = 1; // force refresh if actually expired
                    }
                }

                localStorage.setItem('trakt_token_created_at', createdAt.toString());
                localStorage.setItem('trakt_token_expires_in', expiresIn.toString());
...
async function initSupabaseUI() {
    ...
    // Auto-sync Trakt watch history using smart sync
    // It checks if there are newer activities on Trakt before importing
    if (currentUser && typeof TraktSync !== 'undefined' && typeof TraktAuth !== 'undefined' && TraktAuth.isLoggedIn()) {
        try {
            await TraktSync.syncIfNewer();
        } catch (e) {
            console.warn('[Supabase] Trakt smart sync failed:', e);
        }
    }
}
